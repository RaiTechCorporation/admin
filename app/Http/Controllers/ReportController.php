...
class ReportController extends Controller
{
    ...
    public function acceptUserReport(Request $request){
        ...
        $report = ReportUsers::find($request->id);
        $user = Users::find($report->reported_user_id);
        $user->is_freez = 1;
        $user->save();
    ...
    public function listUserReports(Request $request)
    {
        ...
        $data = $result->map(function ($item) {
            ...
            $user = GlobalFunction::createUserDetailsColumn($item->reported_user_id);
            $reportedBy = GlobalFunction::createUserDetailsColumn($item->user_id);
